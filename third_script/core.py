...
class Link:
    ...
    def generate(self,joints,geometry):
        ...
        names = list(self.pts.keys())

        # Default all points to circular sockets
        socket_by_point = {name: "C" for name in names}

        # Override per-end socket from the JOINTS JSON (new format)
        for J in joints.values():
            if J.link_i is self and J.pt_i_name in socket_by_point:
                socket_by_point[J.pt_i_name] = J.socket_i   # "C" or "S"
            if J.link_j is self and J.pt_j_name in socket_by_point:
                socket_by_point[J.pt_j_name] = J.socket_j   # "C" or "S"

        socket_types = [socket_by_point[name] for name in names]
        ...
        outer_circles = create_joint_features(sketch, pts_xy, socket_types, link_radius, hole_radius)
...
class Joint:
    ...
    def __init__(self, joint_id, link_i, pt_i_name, link_j, pt_j_name,
        ...
        # Per-end socket type
        self.socket_i = (socket_i or "C").upper()
        self.socket_j = (socket_j or "C").upper()
    ...
    def from_json(joint_id, data, links):
        ...
        Old format (no socket):
            "J1": [["L1","02*"], ["L2","02"]]
        """
        # If you ever wrapped joints in a dict like {"ends":[...]} keep this too:
        ...
        link_i_id = e0[0]
        pt_i_name = e0[1]
        socket_i  = e0[2] if len(e0) >= 3 else "C"

        # end 1
        link_j_id = e1[0]
        pt_j_name = e1[1]
        socket_j  = e1[2] if len(e1) >= 3 else "C"

        link_i = links[link_i_id]
        link_j = links[link_j_id]

        return Joint(joint_id, link_i, pt_i_name, link_j, pt_j_name, socket_i=socket_i, socket_j=socket_j)
...
class Mechanism:
    ...
    def connect(self):
        ...
        for joint in self.joints.values():
            ...
            edge_axis = get_revolute_axis_edge_for_joint(joint)
            if edge_axis is None:
                # No circular edge on either side -> can't create revolute axis
                # (likely S/S). Skip or handle as rigid (see below).
                continue
...
def create_joint_features(sketch, pts_xy, socket_types, link_radius, hole_radius):
    """
    Draw outer pad circles at joint centers,
    and inner socket geometry based on socket_types:
      - "C": inner circle (radius = hole_radius)
      - "S": inner square INSCRIBED in that circle
    ...
    for (x, y), st in zip(pts_xy, socket_types):
        st = (st or "C").upper()
        center = adsk.core.Point3D.create(x, y, 0)

        # Outer pad (always circular)
        ...
        if st == "C":
            ...
            # Square inscribed in circle of radius hole_radius:
            # side = sqrt(2) * hole_radius  -> half_side = hole_radius/sqrt(2)
            half_side = hole_radius / math.sqrt(2.0)

            p1 = adsk.core.Point3D.create(x + half_side, y + half_side, 0)
            p2 = adsk.core.Point3D.create(x - half_side, y - half_side, 0)
...
def get_revolute_axis_edge_for_joint(joint):
    """
    Prefer the circular socket end ("C") to define the revolute axis.
    Returns an assembly-context edge or None.
    """
    e_i = None
    e_j = None

    if getattr(joint, "socket_i", "C").upper() == "C":
        e_i = find_joint_hole_edge(joint.link_i, joint.pt_i)

    if getattr(joint, "socket_j", "C").upper() == "C":
        e_j = find_joint_hole_edge(joint.link_j, joint.pt_j)

    # Prefer link_j if available (keeps your old behavior), otherwise link_i
    return e_j or e_i
